...
ImVec2 GetPinCoordinates(const ImPinData& pin)
{
    return pin.Pos;
}
...
void BoxSelectorUpdateSelection(ImNodesEditorContext& editor, ImRect box_rect)
{
    ...
    for (int link_idx = 0; link_idx < editor.Links.Pool.size(); ++link_idx)
    {
        if (editor.Links.InUse[link_idx])
        {
            ...
            const ImVec2 start = GetPinCoordinates(pin_start);
            const ImVec2 end = GetPinCoordinates(pin_end);
...
void ClickInteractionUpdate(ImNodesEditorContext& editor)
{
    ...
    case ImNodesClickInteractionType_LinkCreation:
    {
        ...
        const ImVec2 start_pos = GetPinCoordinates(start_pin);
        // If we are within the hover radius of a receiving pin, snap the link
        // endpoint to it
        const ImVec2 end_pos =
            should_snap
                ? GetPinCoordinates(editor.Pins.Pool[GImNodes->HoveredPinIdx.Value()])
                : GImNodes->MousePos;
